...
int pool[n*n][3]; // three columns: from, to, cost
...
int main(void)
{
    ...
    for (int i=0; i<numb_cust; i++) {
        demand[i][0] = rand() % n; // from 
        demand[i][1] = rand() % n; // to
    }
    ...
    for (int i=1; i<pool_numb; i++) { // i does not start with 0 as [0] is the best plan in the set
        for (int j=0; j<i; j++)
            if (pool[j][0]!=-1 && // do not compare dropped plans, waste of time
            	(pool[i][0]==pool[j][0] || pool[i][1]==pool[j][1] ||
                pool[i][0]==pool[j][1] || pool[i][1]==pool[j][0])) { // the 'i' plan shares one participant with
